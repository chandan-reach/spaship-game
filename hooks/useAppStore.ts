...
export const useAppStore = (): Omit<AppContextType, keyof ReturnType<typeof useDummyHandlers>> => {
  ...
  const shipFileInputRef = useRef<HTMLInputElement>(null);
  ...
  // --- Ship Save/Load Logic ---
  const handleSaveShipConfig = useCallback(() => {
    const data = shipConfig;
    const url = URL.createObjectURL(new Blob([JSON.stringify(data, null, 2)], { type: 'application/json' }));
    const a = document.createElement('a');
    a.href = url;
    a.download = `gemini-ship-config-${uuidv4().slice(0, 8)}.json`;
    document.body.appendChild(a);
    a.click();
    document.body.removeChild(a);
    URL.revokeObjectURL(url);
  ...
  const handleLoadShipConfig = useCallback(() => {
      shipFileInputRef.current?.click();
  }, []);

  const handleShipFileChange = useCallback((e: React.ChangeEvent<HTMLInputElement>) => {
    const file = e.target.files?.[0];
    if (!file) return;
    const reader = new FileReader();
    reader.onload = (ev) => {
      try {
        const loadedConfig = JSON.parse(ev.target?.result as string);
        if (typeof loadedConfig === 'object') {
             setShipConfig(prev => ({
                 ...prev,
                 ...loadedConfig,
                 modulations: (loadedConfig.modulations || []).map((m: any) => ({ ...m, id: m.id || uuidv4() }))
             }));
        }
        if(e.target) e.target.value = '';
      } catch (err) { console.error("Failed to parse ship config file:", err); }
    };
    reader.readAsText(file);
  }, []);
