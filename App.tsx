...
const AppContent: React.FC = () => {
    const {
        ...
        activeShaderCode,
        allUniforms,
        cameraRef, // Needed for direct rotation updates
    ...
    const handlePointerDown = (e: React.PointerEvent) => {
        ...
        // Desktop: Click to lock pointer (FPS style)
        if (e.pointerType === 'mouse') {
             ...
             // Touch: Capture pointer for dragging
             setIsInteracting(true);
    ...
    const handlePointerMove = (e: React.PointerEvent) => {
        ...
        // Logic for calculating delta movement
        if (document.pointerLockElement === target) {
            dx = e.movementX;
            dy = e.movementY;
        } else if (target.hasPointerCapture(e.pointerId)) {
            if (lastPointerPos.current) {
                dx = e.clientX - lastPointerPos.current.x;
                dy = e.clientY - lastPointerPos.current.y;
            }
            lastPointerPos.current = { x: e.clientX, y: e.clientY };
        } else {
            return;
        ...
        // Yaw (Left/Right) - Subtracting dx matches standard FPS mouse look
        cameraRef.current.rotation[1] -= dx * SENSITIVITY;
        
        // Pitch (Up/Down) - Adding dy (mouse down = positive) matches "Mouse Y moves Pitch"
        // Since negative pitch is UP in this engine, dragging DOWN (positive dy) -> Adds to pitch -> Looks DOWN.
        // Dragging UP (negative dy) -> Subtracts from pitch -> Looks UP.
        cameraRef.current.rotation[0] += dy * SENSITIVITY;

        // Clamp Pitch to prevent flipping
        const PITCH_LIMIT = 1.57; // 90 degrees
        cameraRef.current.rotation[0] = Math.max(-PITCH_LIMIT, Math.min(PITCH_LIMIT, cameraRef.current.rotation[0]));
    };
    ...
    return (
        <div className="h-screen w-screen bg-gray-900 text-white flex flex-col overflow-hidden relative">
            ...
            <input
                ...
                accept=".json"
            />
             {/* Hidden input for ship config importing */}
             <input
                type="file"
                ref={shipFileInputRef}
            ...
            <main className={`flex-grow bg-black flex items-center justify-center overflow-hidden`}>
                <div
                    className="relative touch-none cursor-move"
                    style={{ ...canvasContainerStyle, maxWidth: '100%', maxHeight: '100%' }}
                    onPointerDown={handlePointerDown}
                    onPointerMove={handlePointerMove}
                    onPointerUp={handlePointerUp}
                    onPointerCancel={handlePointerUp}
                    title={cameraControlsEnabled ? "Drag or Click to Look Around" : ""}
                >
                    {activeShaderCode && (
                        <ShaderCanvas
